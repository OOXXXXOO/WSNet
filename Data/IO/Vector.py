...
class Vector(Raster):
    ...
    def readshp(self,input_shp_path):
        if os.path.isfile(input_shp_path):
            ...
            for i in range(self.LayerCount):        
                ...
                print("-----Layer :",i," Define : ",self.Layer.GetName(),self.Layer.GetGeometryColumn())
    ...
    def getbounding(self):
        ...
        x1 = adfGeoTransform[0] + 0 * adfGeoTransform[1] + 0 * adfGeoTransform[2]
        y1 = adfGeoTransform[3] + 0 * adfGeoTransform[4] + 0 * adfGeoTransform[5]
        x2 = adfGeoTransform[0] + self.width * adfGeoTransform[1] + self.height * adfGeoTransform[2]
        y2 = adfGeoTransform[3] + self.width* adfGeoTransform[4] + self.height * adfGeoTransform[5]
    ...
    def SaveTo(self,name):
        ...
        if filetype in self.DriveDict.keys():
            out=self.DriveDict[filetype].CopyDataSource(self.DataSource,name)
            out=None
    ...
    def SaveVectorByLayerName(self,LayerName,outputname,format="GeoJSON"):
        ...
        print("# ------------------Start Create Layer",LayerName,' ---------------------- #')
        ...
        assert not ExportResources==None,"Invalid Export Resources ,Please check the path of output"
        ExportLayerTemp=ExportResources.CreateLayer(LayerName,geom_type=ogr.wkbPolygon)
    ...
    def CropLayerByRect(self,rect):
        assert self.ExportLayer==None,'Invalid Export Layer'
    ...
    def Rasterize(self,outputname,Nodata=0):
        ...
        targetDataSet=gdal.GetDriverByName('GTiff').Create(outputname,self.width,self.height,1,gdal.GDT_Byte)
        ...
        band.FlushCache()
        # print("Rastering---")
        gdal.RasterizeLayer(targetDataSet, [1], self.defaultlayer)
        targetDataSet=None
    ...
    def SetFilter(Rect=None,WktPolygon=None):
        ...
        if Rect ==None and WktPolygon==None:
            pass
