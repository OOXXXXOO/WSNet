...
def process(VectorDataSource,
    ...
    DataSetName="Building_Beijing_18",
    Merge=False,
    Keep_local=False,
    ...
    * WgsCord                 : tuple(116.55, 39.9, 116.6, 39.85, 18)
                                    lonmin latmin lonmax latmax zoom 
    
    * Class_key:              : string key of vector layername like 'building'
    * DataSources             : string like "Google"
    * DataSetName             : string like "MydataSet"
    * Merge                   : bool if true , all the small item(include label) will merge to a large imagery 
    * Overwrite               : bool overite the same name remote file on Huawei Obs server  
    * Keep_local              : bool keep the local dataset or delete when upload finished 
    * remote_dataset_root     : string remote obs server root
    * thread_count            : int the downlaod thread count
...
def main():
    ...
    process(
        vecfile,
        WgsCord=(116.55, 39.9, 116.6, 39.85, 18),
        Keep_local=True,
        over_write=True
    )
